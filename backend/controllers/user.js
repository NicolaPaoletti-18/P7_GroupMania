...
exports.profile = (req, res, next) => {
  ...
  mysql.query(sqlGetUser, [userID, userIDAsked], function (err, result) {
    ...
    if (result.length == 0) {
      return res
        .status(400)
        .json({ message: "Aucun utilisateur ne correspond à votre requête" });
    }
...
exports.modify = (req, res, next) => {
  ...
  if (req.file) {
    ...
    // Si le changement concerne les infos de l'user on demande le mdp
        ...
        .then((valid) => {
          ...
          if (newPassword) {
            ...
            bcrypt
              ...
              .then((hash) => {
                sqlChangePassword =
                  "UPDATE User SET email=?, pseudo=?, bio=?, password=? WHERE userID = ?";
                ...
                mysql.query(sqlChangePassword, values, function (err, result) {
                  ...
                  if (result.affectedRows == 0) {
                    return res
                      .status(400)
                      .json({ message: "Changement échoué !" });
                  }
            ...
            sqlModifyUser =
              "UPDATE User SET email=?, pseudo=?, bio=? WHERE userID = ?";
